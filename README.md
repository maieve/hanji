...
현재 설치용 빌드는 [v0.1.15 cloud backup inventory](https://github.com/maieve/hanji/releases/tag/v0.1.15)이다. v0.1.0은 로컬 iOS 모듈이 IPA에 연결되지 않아 시작 시 흰 화면이 발생하므로 사용하지 않는다. 설치 후 설정의 `설치 진단`에서 네이티브 기능 `5/5`를 확인한다. 자세한 설치·갱신 절차는 `docs/IPAD_INSTALL.md`를 따른다.

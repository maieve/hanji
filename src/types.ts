...
export type ToolSpec={kind:ToolKind;color:string;width:number;opacity?:number;eraserMode?:'vector'|'bitmap'|'fixedWidthBitmap';eraserAutoReturn?:boolean;rulerActive?:boolean;lassoMode?:'freeform'|'rectangle';shapeKind?:ShapeKind;shapeLineStyle?:'solid'|'dashed';shapeFillStyle?:'none'|'translucent'|'solid';shapeHoldRequired?:boolean;scratchEnabled?:boolean;circleToLasso?:boolean;markerStraightLine?:boolean};
...
export type Flashcard={id:string;question:string;answer:string;questionImageUri?:string;createdAt:string;updatedAt:string;dueAt:string;intervalDays:number;easeFactor:number;repetitions:number;lapses:number;lastReviewedAt?:string};
export type Notebook={id:string;title:string;folder:string;tags:string[];favorite:boolean;locked?:boolean;coverColor?:string;coverUri?:string;createdAt:string;updatedAt:string;lastOpenedAt?:string;viewMode?:'page'|'continuous';pages:Page[];deletedPages?:Record<string,string>;audioSessions?:AudioSession[];flashcards?:Flashcard[];conflictOf?:string;conflictSignature?:string};

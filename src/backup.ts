...
async function exportArchive(items:Notebook[],prefix:string,dialogTitle:string){
  ...
  for(const note of items){
    ...
    for(const card of note.flashcards??[])if(card.questionImageUri)await addAsset(card.questionImageUri,'flashcard');

...
export async function importLibraryBackupFromUri(uri:string):Promise<Notebook[]>{
  ...
  return restored.map(note=>({...note,pages:note.pages.map(page=>({...page,pdfUri:page.pdfUri?uriMap[page.pdfUri]??page.pdfUri:undefined,customTemplateUri:page.customTemplateUri?uriMap[page.customTemplateUri]??page.customTemplateUri:undefined,elements:page.elements?.map(element=>element.kind==='image'?{...element,uri:uriMap[element.uri]??element.uri}:element)})),audioSessions:note.audioSessions?.map(audio=>({...audio,uri:uriMap[audio.uri]??audio.uri}))}));
}

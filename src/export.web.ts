...
export async function exportNotebookPdf(note: Notebook) {
  const pages = note.pages
    .map((page) => {
      ...
      const paths = strokes
        .map((x) => `<path d="${esc(x.d)}" stroke="${esc(x.color)}" stroke-width="${x.width}" ${x.dashed ? 'stroke-dasharray="12 7"' : ''} opacity="${x.opacity}" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`)
        .join('');
...
export async function exportPagePng(note: Notebook, page: Page, pageIndex: number) {
  ...
  const labels = (page.elements ?? [])
    ...
    .join('');
  const ruled =
    '<defs><pattern id="r" width="28" height="28" patternUnits="userSpaceOnUse"><path d="M0 27.5H28" stroke="#dde2dd"/></pattern><pattern id="g" width="28" height="28" patternUnits="userSpaceOnUse"><path d="M0 27.5H28M27.5 0V28" stroke="#dde2dd"/></pattern><pattern id="d" width="22" height="22" patternUnits="userSpaceOnUse"><circle cx="1" cy="1" r="1" fill="#bdc4bd"/></pattern></defs>';
  const background = page.customTemplateUri
    ? `<image href="${esc(page.customTemplateUri)}" width="900" height="636" preserveAspectRatio="none"/>`
    : page.template === 'dark'
      ? '<rect width="900" height="636" fill="#202522"/><path d="M0 28H900M0 56H900M0 84H900M0 112H900M0 140H900M0 168H900M0 196H900M0 224H900M0 252H900M0 280H900M0 308H900M0 336H900M0 364H900M0 392H900M0 420H900M0 448H900M0 476H900M0 504H900M0 532H900M0 560H900M0 588H900M0 616H900" stroke="#465149"/>'
      : page.template === 'cornell'
        ? `${ruled}<rect width="900" height="636" fill="white"/><rect x="225" width="675" height="521" fill="url(#r)"/><path d="M225 0V521M0 521H900" stroke="#bfd0c8"/>`
        : page.template === 'planner'
          ? `<rect width="900" height="636" fill="white"/><path d="${Array.from({ length: 8 }, (_, i) => `M${i * 128.57} 60V620`).join(' ')} ${Array.from({ length: 10 }, (_, i) => `M0 ${60 + i * 62}H900`).join(' ')}" stroke="#dde2dd"/>`
          : page.template === 'line'
            ? `${ruled}<rect width="900" height="636" fill="url(#r)"/>`
            : page.template === 'grid'
              ? `${ruled}<rect width="900" height="636" fill="url(#g)"/>`
              : page.template === 'dot'

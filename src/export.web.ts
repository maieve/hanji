...
export async function exportNotebookPdf(note: Notebook) {
  const pages = note.pages
    .map((page) => {
      ...
      return `<section class="page ${portrait ? "portrait" : "landscape"}"><div class="page-content ${page.template}" style="--spacing:${spacing}px;transform:translate(-50%,-50%) rotate(${rotation}deg)">${custom}${paint}<svg viewBox="0 0 900 636">${paths}</svg>${elements}</div></section>`;
    })
    .join("");
...
export async function exportPagePng(
  ...
  const background = page.customTemplateUri
    ...
    : page.template === "dark"
      ? `<rect width="900" height="636" fill="#202522"/><path d="${Array.from({length:Math.ceil(636/spacing)},(_,i)=>`M0 ${(i+1)*spacing}H900`).join(' ')}" stroke="#465149"/>`
  ...
  const paint =
    page.backgroundColor && (page.backgroundOpacity ?? 0) > 0
      ? `<rect width="900" height="636" fill="${esc(page.backgroundColor)}" opacity="${page.backgroundOpacity}"/>`
      : "";
  const rotation = page.rotation ?? 0,
    ...
    transform =
      rotation === 90
        ? "translate(636 0) rotate(90)"
        : rotation === 180
          ? "translate(900 636) rotate(180)"
          : rotation === 270
            ? "translate(0 900) rotate(-90)"
            : "";

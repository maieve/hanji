...
export function duplicateNotebookPage(note:Notebook,pageId:string):Notebook{
 ...
 return{...note,pages:[...note.pages.slice(0,index+1),copy,...note.pages.slice(index+1)],audioSessions:appendAudio(note.audioSessions??[],incomingAudio),updatedAt:timestamp};
}

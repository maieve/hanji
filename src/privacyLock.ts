...
export function usePrivacyLock(){
 ...
 const authenticate=async(promptMessage='Hanji 잠금 해제')=>{if(Platform.OS==='web')return true;const result=await LocalAuthentication.authenticateAsync({promptMessage,cancelLabel:'취소',disableDeviceFallback:false});if(result.success)setLocked(false);return result.success};

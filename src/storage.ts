...
export async function loadCategories():Promise<string[]>{const raw=await AsyncStorage.getItem(CATEGORY_KEY);try{return raw?JSON.parse(raw):['내 노트','업무','스터디','개인']}catch{return['내 노트','업무','스터디','개인']}}
export async function saveCategories(v:string[]){await AsyncStorage.setItem(CATEGORY_KEY,JSON.stringify(v))}

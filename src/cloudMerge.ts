...
const mergeAudio=(a:AudioSession[]=[],b:AudioSession[]=[])=>{
 ...
 for(const session of b){const current=result.get(session.createdAt);if(!current||session.durationMs>current.durationMs||(!current.transcript&&session.transcript))result.set(session.createdAt,session)}
 return [...result.values()].sort((x,y)=>x.createdAt.localeCompare(y.createdAt));
};

const mergeCards=(a:Flashcard[]=[],b:Flashcard[]=[])=>{
 const result=new Map(a.map(card=>[card.id,card]));
 for(const card of b){const current=result.get(card.id);result.set(card.id,current?newer(current,card):card)}
 return [...result.values()];
};

type ConflictPage={page:Page;source:Notebook};

function mergeNotebook(local:Notebook,remote:Notebook):{note:Notebook;conflicts:ConflictPage[]} {
 ...
 const pages=order.flatMap(id=>{
  ...
  const remoteWins=right.updatedAt>left.updatedAt||(right.updatedAt===left.updatedAt&&remote.updatedAt>local.updatedAt);
...
function conflictNotebook(sourceId:string,items:ConflictPage[]):Notebook {
 const first=items[0]!;
 const base=items.reduce((latest,item)=>item.source.updatedAt>latest.updatedAt?item.source:latest,first.source);
 const content=items.map(item=>`${item.page.id}:${signature(pageContent(item.page))}`).sort().join('|');
 const conflictSignature=signature(content),stamp=`${Date.now()}-${conflictSignature}`;
...
export function mergeCloudRestore(local:Notebook[],remote:Notebook[]):Notebook[]{
 ...
 for(const incoming of remote){
  ...
  if(current.conflictSignature||incoming.conflictSignature){if(incoming.updatedAt>current.updatedAt)merged.set(incoming.id,incoming);continue}
  const result=mergeNotebook(current,incoming);
  merged.set(incoming.id,result.note);
  const novelConflicts=result.conflicts.filter(({page})=>!existingConflicts.some(note=>note.conflictOf===incoming.id&&note.pages.some(saved=>pageContent(saved)===pageContent(page))));
  if(novelConflicts.length){
   const copy=conflictNotebook(incoming.id,novelConflicts);
   if(!existingConflicts.some(note=>note.conflictOf===incoming.id&&note.conflictSignature===copy.conflictSignature)){existingConflicts.push(copy);merged.set(copy.id,copy)}
  }
 }

...
export async function listCloudBackups(config:CloudConfig){const response=await fetch(`${endpoint(config.endpoint)}/v1/backups`,{headers:headers(config)});if(!response.ok)throw new Error(`목록 조회 실패 (${response.status})`);const objects=(await response.json() as {objects:CloudBackup[]}).objects;return [...objects].sort((a,b)=>new Date(b.uploaded).getTime()-new Date(a.uploaded).getTime())}

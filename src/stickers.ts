...
export function stickerFromImage(element:ImageElement):Sticker{return{id:`sticker-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,name:`스티커 ${new Date().toLocaleDateString('ko-KR')}`,uri:element.uri,width:element.width,height:element.height,fit:element.fit,rotation:element.rotation,cropZoom:element.cropZoom,cropX:element.cropX,cropY:element.cropY,createdAt:new Date().toISOString()}}

...
export async function exportPagePng(note:Notebook,page:Page,pageIndex:number){
  ...
  await native.exportPNG({drawingData:page.drawingData,pdfUri:page.pdfUri??'',pdfPageIndex:String(page.pdfPageIndex??pageIndex),template:page.template,templateSpacing:page.templateSpacing??'medium',backgroundColor:page.backgroundColor??'',backgroundOpacity:String(page.backgroundOpacity??0),rotation:String(page.rotation??0),customTemplateUri:page.customTemplateUri??'',elements:JSON.stringify(page.elements??[])},file.uri);

...
type Props={count:number;color:string;availableWidth:number;onRecolor:()=>void;onCopy:()=>void;onClip:()=>void;onImageFlashcard:()=>void;onCut:()=>void;onDuplicate:()=>void;onShrink:()=>void;onGrow:()=>void;onRotate:()=>void;onText:()=>void;onFlashcard:()=>void;onDelete:()=>void;onClose:()=>void};
...
export function SelectionBar({count,color,availableWidth,onRecolor,onCopy,onClip,onImageFlashcard,onCut,onDuplicate,onShrink,onGrow,onRotate,onText,onFlashcard,onDelete,onClose}:Props){
 ...
 return <View accessibilityLabel={`${count}개 획 선택됨`} style={[s.bar,{width,transform:[{translateX:-width/2}]}]}>
  ...
  <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.row}>
   ...
   <Action label="선택 필기를 이미지 플래시카드로 만들기" icon="image-outline" onPress={onImageFlashcard}/>
   ...
   <Action label="선택 필기를 플래시카드 질문으로 만들기" icon="albums-outline" onPress={onFlashcard}/>
   <Action label="선택 획 삭제" icon="trash-outline" color={C.danger} onPress={onDelete}/>
  </ScrollView>
 </View>;
}

const s=StyleSheet.create({bar:{position:'absolute',top:16,left:'50%',zIndex:30,minHeight:82,borderRadius:16,backgroundColor:'rgba(255,255,255,.97)',borderWidth:1,borderColor:C.line,shadowColor:'#000',shadowOpacity:.15,shadowRadius:12,padding:7,gap:4},row:{height:32,minWidth:'100%',paddingHorizontal:2,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:6},count:{minWidth:70,fontSize:11,fontWeight:'800',color:C.ink},action:{width:42,height:32,borderRadius:9,backgroundColor:C.accentSoft,alignItems:'center',justifyContent:'center'},color:{width:16,height:16,borderRadius:8,borderWidth:1,borderColor:C.white},close:{width:32,height:32,alignItems:'center',justifyContent:'center'}});

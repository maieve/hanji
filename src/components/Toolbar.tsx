...
export function Toolbar({ tool, setTool, onLibrary, title, onTitleChange, onAddPage }: { tool: ToolSpec; setTool: (v: ToolSpec) => void; onLibrary: () => void; title: string; onTitleChange: (v: string) => void; onAddPage: () => void }) {
  return <View style={s.bar}>
    ...
    <ScrollView horizontal contentContainerStyle={s.tools} showsHorizontalScrollIndicator={false}>
      ...
      <View style={s.colors}>{colors.map(color => <Pressable key={color} onPress={() => setTool({ ...tool, color })} style={[s.color, { backgroundColor: color }, tool.color === color && s.colorSelected]} />)}</View><TextInput accessibilityLabel="HEX 색상" value={tool.color} autoCapitalize="characters" maxLength={7} onChangeText={color=>{if(/^#[0-9A-Fa-f]{0,6}$/.test(color))setTool({...tool,color})}} style={s.hex} />
      {[1, 2, 5, 9, 16, 28].map(width => <Pressable key={width} onPress={() => setTool({ ...tool, width })} style={[s.width, tool.width === width && s.selected]}><View style={{ width: width + 3, height: width + 3, borderRadius: 20, backgroundColor: C.ink }} /></Pressable>)}<View style={{flexDirection:'row',alignItems:'center',gap:3,marginLeft:4}}>{[0.25,0.5,0.75,1].map(opacity=><Pressable accessibilityLabel={`투명도 ${opacity*100}%`} key={opacity} onPress={()=>setTool({...tool,opacity})} style={[s.opacity,tool.opacity===opacity&&s.selected]}><Text style={s.opacityText}>{opacity*100}</Text></Pressable>)}</View>
    </ScrollView>

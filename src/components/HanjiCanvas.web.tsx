...
import {closedPolygon,SHAPE_HOLD_MS,shouldSnapShape,supportsShapeFill,type ShapePoint} from '../shapePolicy';
...
const polygonPath=(points:ShapePoint[],width:number)=>{const polygon=closedPolygon(points,Math.max(3,width*.75));return polygon.length<4?'':polygon.map((point,index)=>`${index?'L':'M'} ${point.x} ${point.y}`).join(' ')+' Z'};
...
export function HanjiCanvas({ drawingData, tool, onDrawingChange }: HanjiCanvasProps) {
  ...
  const shapePoints=useRef<ShapePoint[]>([]);
  ...
  const responder = useMemo(() => PanResponder.create({
    ...
    onPanResponderGrant: ({ nativeEvent }) => {
      ...
      if(tool.kind==='shape'){shapeStart.current={x:nativeEvent.locationX,y:nativeEvent.locationY};shapePoints.current=[shapeStart.current]}
    ...
    onPanResponderMove: ({ nativeEvent }) => {
      ...
      if(tool.kind==='shape'&&shapeStart.current){shapePoints.current.push(strokeEnd.current);rawShapePath.current+=` L ${nativeEvent.locationX} ${nativeEvent.locationY}`;live.current.d=rawShapePath.current}else live.current.d += ` L ${nativeEvent.locationX} ${nativeEvent.locationY}`;
      redraw(v => v + 1);
    },
    onPanResponderRelease: () => {
      if (tool.kind === 'eraser'||tool.kind === 'lasso') return;
      if (!live.current) return;
      if(tool.kind==='shape'&&shapeStart.current&&strokeEnd.current&&shouldSnapShape(tool.shapeHoldRequired??true,Date.now()-lastMoveAt.current>=SHAPE_HOLD_MS)){const kind=tool.shapeKind??'line',path=kind==='polygon'?polygonPath(shapePoints.current,tool.width):shapePath(kind,shapeStart.current.x,shapeStart.current.y,strokeEnd.current.x,strokeEnd.current.y);if(path)live.current.d=path;if(supportsShapeFill(kind)&&tool.shapeFillStyle&&tool.shapeFillStyle!=='none')live.current.fill=tool.shapeFillStyle}
      ...
      const next = [...strokes, live.current]; live.current = null; shapeStart.current=null;shapePoints.current=[];rawShapePath.current=''; setStrokes(next); onDrawingChange(JSON.stringify(next));
    },

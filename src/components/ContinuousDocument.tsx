...
export function ContinuousDocument(props:Props){
 ...
 const pageWidth=Math.max(320,Math.min(900,layoutWidth-48));const dimensions=(page:Page)=>page.rotation===90||page.rotation===270?{width:pageWidth/1.414,height:pageWidth}:{width:pageWidth,height:pageWidth/1.414};const itemHeight=(page:Page)=>dimensions(page).height+38;const itemOffset=(index:number)=>props.pages.slice(0,index).reduce((sum,page)=>sum+itemHeight(page),0);
 useEffect(()=>{if(visibleIndex.current===props.activeIndex)return;visibleIndex.current=props.activeIndex;const timer=setTimeout(()=>list.current?.scrollToIndex({index:Math.min(props.activeIndex,props.pages.length-1),animated:false}),50);return()=>clearTimeout(timer)},[props.activeIndex,props.pages.length,pageWidth]);

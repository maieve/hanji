  ...
  const render = ({ item, index }: { item: Page; index: number }) => {
    ...
    return (
      <View style={[s.item, { height: itemHeight(item) }]}>
        <ZoomablePage rotation={item.rotation} canvasExtent={item.canvasExtent} fingerDrawingEnabled={props.fingerDrawingEnabled} zoomWindowEnabled={props.zoomWindowEnabled&&index===props.activeIndex} style={[s.paper, size]}>
          <Paper
